...
//    [CIColor(241, 241, 241) set];
    ...
    [[self returncolorRed:1 andBlue:0 andGreen:1]  set]; // 进度环颜色
...
//    [CIColor(244, 81, 0) set];
    [[self returncolorRed:1 andBlue:0 andGreen:0]  set]; // 已完成进度颜色
    CGContextMoveToPoint(ctx, xCenter, yCenter);
    //CGContextAddLineToPoint(ctx, xCenter, 0);
    CGFloat to =M_PI*1.5+ (1-self.progress) * M_PI * 2 + 0.001; // 初始值 （去掉了负号“-”）
    CGContextAddArc(ctx, xCenter, yCenter, radius, M_PI*1.5, to, 1); // 反方向运行
    CGContextClosePath(ctx);
    CGContextFillPath(ctx);
    
    // 遮罩
    [[UIColor whiteColor] set];
    CGFloat maskW = (radius - 15) * 5;
    CGFloat maskH = maskW;
    ...
    attributes[NSFontAttributeName] = [UIFont boldSystemFontOfSize:25 * SDProgressViewFontScale];
...
-(UIColor *) returncolorRed:(CGFloat)red andBlue:(CGFloat)blue andGreen:(CGFloat)green
{
